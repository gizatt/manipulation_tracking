...
int main(int argc, char** argv) {
  ...
  for (size_t k=0; k<scene_pts_tf->size(); k++){
    prog.AddLinearConstraint(C1, 1, 1, {C.block(k, 0, 1, model_pts_tf->size()).transpose()});
  }
  ...
  for (int k_s=0; k_s<scene_pts_tf->size(); k_s++){
    for (int k_m=0; k_m<model_pts_tf->size(); k_m++){
      prog.AddLinearCost( drake::Vector1d(pointDistance(scene_pts_tf->at(k_s), model_pts_tf->at(k_m))), {C.block<1,1>(k_s, k_m)} );
    }
  }
