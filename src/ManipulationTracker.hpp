...
std::shared_ptr<RigidBodyTree<double>> setupRobotFromConfig(YAML::Node config, Eigen::VectorXd& x0_robot, std::string base_path, bool verbose = false, bool less_collision = false);
...
public:
  ...
  ManipulationTracker(std::shared_ptr<const RigidBodyTree<double>> robot, Eigen::Matrix<double, Eigen::Dynamic, 1> x0_robot, std::shared_ptr<lcm::LCM> lcm, YAML::Node config, bool verbose = false);
  ~ManipulationTracker() {};
  ...
  void update();
  std::shared_ptr<const RigidBodyTree<double>> getRobot() { return robot_; };
...
private:
  std::shared_ptr<const RigidBodyTree<double>> robot_;
